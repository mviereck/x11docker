...
| `--xpra` | Seamless integration of application windows on host display. Best `--clipboard` support, picture clips are possible.  Seamless scaling (0,5x, 1.5x, 3x ...) with option `--scale`. |
...
| `--xdummy` | Invisible X server for custom access. Output of environment variables on stdout. With --gpu a setup with Weston, Xwayland and xdotool is used (instead of Xdummy or Xvfb). |
...
| `--kwin-xwayland` | Like --weston-xwayland, but using KWin instead of Weston. Runs in X, in Wayland and from console. |
